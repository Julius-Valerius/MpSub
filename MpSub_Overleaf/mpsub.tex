...
Theorem~\ref{thm:liminf_convergence} establishes almost-sure global convergence in the subsequential sense, which is the primary convergence guarantee in derivative-free and randomized trust-region methods \cite{conn2009introduction,conn2000trust}.

To investigate under what conditions the entire sequence of gradient norms converges to zero, fix any $\varepsilon>0$ and define the index set of large gradients:
\begin{equation}
...
Together with Lemma~\ref{lem:radius_summability}, these moment bounds imply
\begin{equation}
...
Theorem~\ref{thm:liminf_convergence} (and Theorem~\ref{thm:global_convergence}) establishes first-order global convergence for a fixed deterministic objective. In our LLM fine-tuning experiments, minibatches change across iterations, so asymptotic convergence to a stationary point of the population loss is not claimed.
