...
For smooth deterministic objectives, we analyze the Gaussian directions directly without orthogonality assumptions. We derive finite-difference error bounds, quantify the gradient captured by the random subspace, and show that the gradient norm converges to zero almost surely under a safeguarded trust-region radius update. We then compare \texttt{MpSub} with MeZO on OPT-125M and OPT-350M under matched forward-evaluation budgets.
...
At each iteration $k$, \texttt{MpSub} maintains the parameter iterate $\bx_k \in \mathbb{R}^n$, a trust-region radius $\Delta_k \in [\Delta_{\min}, \Delta_{\max}]$, and a momentum displacement $\bm m_k \in \mathbb{R}^n$ generated by the most recent accepted step. The radius $\Delta_k$ serves as both the finite-difference sampling scale and the trust-region bound, and $\bm m_k$ is the momentum displacement from the most recent accepted step. The overall procedure is summarized in Algorithm~\ref{alg:mpsub}.
...
\end{equation}
with the same distribution supplying $\bd_1^{(k)}$ when no accepted step is available. The factor $1/\sqrt{n}$ normalizes the expected Euclidean norm, $\mathbb E\|\bd_i^{(k)}\|_2^2 = 1$.
...
\label{eq:near_ortho}
\end{equation}
Consequently, the pairwise inner products have root-mean-square magnitude $n^{-1/2} \approx 10^{-4}$ at $n \sim 10^8$. Since high-dimensional Gaussians are nearly orthogonal, we omit Gram--Schmidt orthogonalization to save computation and memory; the convergence of \texttt{MpSub} with unorthogonalized directions is established in Section~\ref{sec:analysis}. The dimension $p$ controls how many directional derivatives enter the subspace model; its choice is examined empirically in Section~\ref{sec:experiments}.
...
Across iterations, a fresh minibatch $\mathcal B_{k+1}$ is drawn, while $\Delta_k$ and $\bm m_k$ carry over to the next step. Each iteration expends $2p + 2$ forward passes on the training objective ($42$ passes when $p = 20$).
...
To avoid this memory overhead, \texttt{MpSub} never materializes the matrix $\bm D_k$ in memory. Instead, each exploratory direction $\bd_i^{(k)}$ ($i = 2, \ldots, p$) is generated on the fly using a deterministic seed $\operatorname{seed}(k, i) = (\operatorname{base\_seed} \cdot C_1 + k \cdot C_2 + i \cdot C_3) \bmod M$ with fixed primes $C_1, C_2, C_3$ and Mersenne prime $M = 2^{31}-1$. Weights are perturbed, evaluated, and restored in place, reconstructing directions as needed without storing them.
...
Training is thus executed through forward inference passes alone. Auxiliary storage is confined to the momentum vector $\bm m_k$ and a single trial-step buffer of size $n$, keeping the training memory footprint at the inference level and independent of the subspace dimension $p$.
...
\end{proof}
We now analyze the gradient captured by the Gaussian directions in Algorithm~\ref{alg:mpsub}. Let $\mathcal F_k$ contain the algorithmic history before the fresh exploratory directions at iteration $k$ are sampled. Conditionally on $\mathcal F_k$,
\[
...
To prove \eqref{eq:good_frame_probability}, fix $q\in(0,1)$. Since a $\chi^2_{p-1}$ random variable is positive almost surely, we can choose $\tau>0$ sufficiently small that $\mathbb P(\chi^2_{p-1}\ge\tau)$ is arbitrarily close to one. Setting $\alpha=\sqrt{\tau/n}$, equation~\eqref{eq:chi_square_capture} implies
\[
...
To establish full convergence, fix any $\varepsilon>0$ and define the index set of large gradients:
\begin{equation}
...
\]
Since the conditioning events $G_k$ satisfy $\mathbb P(G_k\mid\mathcal F_k)\ge q > 1/2$ uniformly, applying the renewal property for probabilistically accurate models \cite{cartis2024randomizedsubspacederivativefreeoptimization} along the index subsequence $\mathcal K_\varepsilon$ implies that the sum over the entire set $\mathcal K_\varepsilon$ is finite:
\[
\sum_{k\in\mathcal K_\varepsilon}\Delta_k < \infty \qquad \text{almost surely}.
\]
\end{proof}

Let $\beta_k \coloneqq \|\bm D_k\|_2$. Because $\bm D_k$ contains one unit vector and $p-1$ independent normalized Gaussian vectors $\bz_i/\sqrt{n}$, standard spectral norm bounds for Gaussian random matrices ensure that
\[
...
MeZO is sensitive to its learning rate: varying $\eta_{\mathrm{M}}$ by one order of magnitude from $10^{-6}$ reduces test accuracy by $0.048$ at $10^{-7}$ and by $0.084$ at $10^{-5}$, requiring a grid search ($42{,}000$ forward passes on OPT-125M and $25{,}200$ on OPT-350M) to find an effective rate.

In contrast, \texttt{MpSub} shows little variation across the tested range: mean test accuracy remains between $0.655$ and $0.678$ across all five initial radii from $\Delta_0 = 10^{-3}$ to $3\times 10^{-1}$. Even when $\Delta_0$ is initialized conservatively at $10^{-3}$, successful steps double $\Delta_k$ until it reaches a scale where progress can be made. The default choice $\Delta_0 = 10^{-1}$ therefore works across both model sizes without retuning.
...
As $p$ increases from 5 to 20, development accuracy rises from $0.70$ to $0.78$ and test accuracy from $0.643$ to $0.679$, consistent with the gradient capture bound in Lemma~\ref{lem:capture}. Beyond $p = 20$, accuracy plateaus while evaluation cost and per-step latency continue to increase (from $3.0$ seconds at $p = 5$ to $10.1$ seconds at $p = 30$). For the budget considered here, $p = 20$ captures most of the performance gain while keeping per-step cost moderate.

\section{Conclusion}
\label{sec:conclusion}

We proposed \texttt{MpSub}, which combines a random subspace with trust-region step-size control for derivative-free LLM fine-tuning. For smooth deterministic objectives, we showed that the gradient norm converges to zero almost surely under Gaussian direction sampling and the safeguarded radius update. On CommitmentBank, \texttt{MpSub} achieved results comparable to tuned MeZO on OPT-125M and OPT-350M under the same forward-pass budget, while using the same trust-region settings for both models.
