...
class OurArguments(TrainingArguments):
    ...
    # psub: p-dim subspace trust-region solver (replaces legacy 2D-MoSub)
    use_psub: bool = True      # False -> fall back to legacy 2D solvetry
    psub_p: int = 20           # subspace dimension (sweet spot 10-30)
    psub_nsteps: int = 1       # inner iterations per batch
    psub_delta0: float = 1e-1  # initial trust-region radius (the only real knob)
    psub_dmax: Optional[float] = None  # radius cap; None -> 10*delta0 (step safety)
    psub_eta: float = 0.1      # success threshold for rho
    psub_kind: str = "grad"    # 'grad' (recommended) | 'diag' (curvature, noisy)
    psub_ortho: bool = True    # Gram-Schmidt; ignored in stream mode
    psub_stream: bool = True   # O(n) memory; required for full-parameter tuning
    psub_use_torch: bool = False  # GPU fast path: perturb params on GPU, ~8-15x faster
    ...
    def load_model(self):
        ...
        with count_time("Loading model with FP%d" % (16 if self.args.load_float16 else 32)):
            if torch.cuda.is_available():
                ...
                free_in_GB = 8  # A safe fallback for Macs/CPUs
            ...
            if self.args.head_tuning:
                ...
                model = AutoModelForCausalLM.from_pretrained(
                    self.args.model_name if self.args.model_path is None else self.args.model_path,
                    config=config,
                    # device_map must be 'auto' so that weights and inputs
                    # share a device.
                    device_map='auto',
...
def main():
    ...
    # Optional local-run safeguard: cap this process's CUDA caching allocator
    # so interactive desktop applications retain a fixed share of VRAM.  The
    # paper experiments leave this unset; it changes memory allocation policy,
    # not the optimizer or numerical configuration.
    _cuda_fraction = os.environ.get("MPSUB_CUDA_MEMORY_FRACTION")
    if _cuda_fraction and torch.cuda.is_available():
        _cuda_fraction = float(_cuda_fraction)
        if not 0.0 < _cuda_fraction <= 1.0:
            raise ValueError("MPSUB_CUDA_MEMORY_FRACTION must lie in (0, 1].")
        torch.cuda.set_per_process_memory_fraction(_cuda_fraction, device=0)
        print("[cuda] per-process allocator fraction=%.3f" % _cuda_fraction,
              flush=True)
