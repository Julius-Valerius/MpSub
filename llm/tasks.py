...
class CBDataset(Dataset):
    ...
    def load_dataset(self, path, **kwargs):
        # Prefer local parquet files when present.
        _cb_dir = os.path.join(os.path.dirname(__file__), 'data', 'cb')
        if os.path.isdir(_cb_dir):
            d = load_dataset("parquet", data_files={
                "train": os.path.join(_cb_dir, "train.parquet"),
                "validation": os.path.join(_cb_dir, "validation.parquet"),
            })
        else:
